...
const TeamSectionLeft: FC<{
  team: TeamInterface;
  showRightHandler: () => void;
  teamMessage: string;
  showRight: boolean;
  teamMessageChangeHandler: () => void;
  teamSendMessageHandle: () => void;
  teamOnEmojiClick: () => void;
  showTopics: () => void;
  // Projects
  showProjects: () => void;
  ...
  showMenu: () => void;
  // Files
  handleChooseFileIconTeam: () => void;
  handleChooseFileIcon2Team: () => void;
  handleChooseFileTeam: () => void;
  handleRemoveFileTeam: () => void;
  teamFileInput: any;
  teamFileInput2: any;
  teamFiles: FileInterface[];
  topicTitle: string;
  topicTitleChangeHandler: () => void;
  topicDescriptionChangeHandler: () => void;
  createTopicHandler: () => void;
  startTopic: boolean;
  toggleTopicHandler: () => void;
  // Topic
  handleClickedTopic: () => void;
  goToTopic: () => void;
  //
  topicsDisplay: boolean;
...
const Mid: FC<{
  ...
  handleShowTeamFile: (file: FileInterface) => void;
...
const TopicMessage: FC<{
  message: TeamMessageInterface;
  user: UserInterFace;
  handleShowTeamFile: (file: FileInterface) => void;
  handleClickedTopic: (id: string) => void;
  goToTopic: () => void;
...
const MessageComponentForTopic: FC<{
  message: TeamMessageInterface;
  user: UserInterFace;
  handleShowTeamFile: (file: FileInterface) => void;
  textColor: string;
}> = ({ message, user, handleShowTeamFile, textColor }) => {
  const idProvided = message.id ? true : false;
  const theme: ThemeInterface = useTheme();
  return (
    <Box
...
const ColleagueMessage: FC<{
  ...
  handleClickedTopic: (id: string) => void;
  goToTopic: () => void;
}> = ({ message, handleShowTeamFile, handleClickedTopic, goToTopic }) => {
  const theme: ThemeInterface = useTheme();
  const router = useRouter();
  const id = router.query.id;
  const userStore = useSelector((state: RootState) => state.user);
...
const Bottom: FC<{
  ...
  teamSendMessageHandle: (id: string) => void;
  teamMessage: string;
  teamOnEmojiClick: () => void;
  // Files
  handleChooseFileIconTeam: () => void;
  handleChooseFileIcon2Team: (e: React.MouseEvent<HTMLButtonElement>) => void;
  handleChooseFileTeam: (e: React.ChangeEvent<HTMLInputElement>) => void;
  handleRemoveFileTeam: (file: FileInterface) => void;
  teamFileInput: any;
  teamFileInput2: any;
  teamFiles: FileInterface[];
  // Topic
          ...
          <OutlinedInput
            ...
            endAdornment={
              <InputAdornment position="end">
                <IconButton
                  onClick={() => teamSendMessageHandle(topicId as string)}
                  edge="end"
                >
                  <svg
